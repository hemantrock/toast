var ddlTankModes = ""; ddlSeqOptions = ""; var sameGrptags = "";
var LevelTag = "_LEVEL";
var VolumeTag = "_TOV";
...
function ExtraActivities(model) {
    $.each(model, function (i, item) {
        //if (item.ControlName.toUpperCase().indexOf('AUTOMAN') >= 0 && item.ControlName.toUpperCase().indexOf('TK') >= 0 && item.Value != null && item.Value != undefined) {
        ...
        if ((item.TagName.toUpperCase().indexOf('PER_LVL') >= 0 || item.TagName.toUpperCase().indexOf('PER_MAX') >= 0 || item.ControlName.toUpperCase().indexOf('PER_HGHT') >= 0)
...
function UpdateTankFill(item) {
    ...
    var tankNo = item.ControlName.replace('_PER_LVL', '').replace('_PER_MAX', '').replace('_PER_HGHT', '').toUpperCase()
    $('.' + tankNo + '_OIL_TANK_FILL .oil-tank-fill').css('height', (perc).toFixed(0) + '%');
    var svgFillTag = tankNo + '_SVG_OIL_TANK_FILL';
    var svgFillEle = $('#' + svgFillTag);
    if (svgFillEle != undefined || svgFillEle != null) {
        ...
        success: function (result) {
            if (result.blnStatus != 'undefined' && result.blnStatus == false) {
                if (result.strType == "danger") {
                    ...
                    if (result.blnStatus == true) {
                        opc.server.sendDataToOPC(TagName, existingValue, opcPageName, CurrUserID, "", "", NewVal, true, false);
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.blnStatus != 'undefined' && result.blnStatus == false) {
                ...
                SetProcessValue(activityName, activityDetail, src, SeqTagName, "DDL", UserID, LoginID, UserName);
            }
        }
    });
}
