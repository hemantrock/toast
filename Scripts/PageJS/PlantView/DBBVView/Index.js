$(document).ready(function () {
    //opcPageName = "DBBVView.*";
    InitConn(opcPageName, 0);
    opc.server.readPageTags(opcPageName);
    createTooltip();
});

function OpenMOVModal(DBBVId, PageName) {
    $.ajax({
        type: "GET",
        url: "/PlantView/GetDBBVDetailsView?DBBVId=" + DBBVId,
        cache: false,
...
function ValidateOpenClose(src, MovName, TagToWrite, UserID, LoginID, UserName) {
    ...
    var activityName = "Validate Open Close DBBV";
    var activityDetail = "DBBV: " + MovName;
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.blnStatus != 'undefined' && result.blnStatus == false) {
                ...
                ShowPasswordModalForReqTrn('Change Auto/Manual Mode DBBV', function (ApproverId) {
                    SendDataToOPC(src, TagToWrite, UserID, LoginID, UserName, currValDesc);
...
function ValidateOpenPercent(src, MovName, TagToWrite, UserID, LoginID, UserName) {
    ...
    var activityName = "Validate Open Percent DBBV";
    var activityDetail = "DBBV: " + MovName;
...
function MaintainenceMode(MMDetails, src, TagName, UserID, LoginID, UserName, event) {
    ...
    cancelBubble(event);
    var activityName = "Set DBBV Maintainence Mode";
    var activityDetail = "DBBV Maintainence Mode: " + MMDetails;
