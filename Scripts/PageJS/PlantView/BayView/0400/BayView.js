...
{
    if (model.length > 50) { pageLoaded = false; };
    CheckNoCard(model);
    UpdateLoadingArm(model);
    UpdatePercentage();
    if (model.length > 50) { pageLoaded = true; };
}
...
function CheckNoCard(model)
{
    ...
    $.each(model, function (i, item) {
        if (item.ControlName.toUpperCase().indexOf('CARD_NO') >= 0 && item.Value != null && item.Value != undefined) {
            toRefresh = true;
        }

        if (item.ControlName.toUpperCase().indexOf('CURR_CHAMBER') >= 0 && item.Value != null && item.Value != undefined) {
            toRefresh = true;
        }
    });
    
    if (pageLoaded && toRefresh) {
        RefreshData();
    }
}

function UpdateLoadingArm(model) {
    var toAnimate = false;

    $.each(model, function (i, item) {
        if (item.ControlName.toUpperCase().indexOf('LAPS') >= 0 && item.Value != null && item.Value != undefined) {
            toAnimate = true;
            pipeAnimation(toAnimate);
        }
        if (item.ControlName.toUpperCase().indexOf('QTY_ACC_CH') >= 0)
        {
            ...
            if (Effpreset != undefined)
            {
                ...
                {
                    $('.' + item.ControlName.replace("QTY_ACC", "QTY_REM")).text(parseFloat(Effpreset - delivered));
                }
                else
                {
...
function RemoteStopDo(src, ControlName, LPNo, UserID, LoginID, UserName) {
    ...
    $.ajax({
        type: "GET",
        url: "/BatchController/cmdRemoteStop?LPNo=" + LPNo + "&CurrRemStop=" + CurrRemStop + "&TagName=" + TagName,
        cache: false,
        success: function (result) {
            if (result.blnStatus != 'undefined' && result.blnStatus == false) {
                alertDismissable(result.strType, result.strMessage);
