...
//    tblObj = $('#tblDataSource').DataTable({
        ...
        success: function (result) {
            ...
            $("#jqxgrid").jqxGrid(
            {
                ...
                rendertoolbar: function (statusbar) {
                    ...
                    var tableHeading = $("<div style='float: left; margin-left: 5px;' class='heading-lbl'><span style='margin-left: 4px; position: relative; top: -3px;'>Data Source Master</span></div>");
                ...
                columns: [
                  { text: 'DS Name', datafield: 'DSName', width: "20%", filtertype: "custom", createfilterpanel: function (datafield, filterPanel) { buildFilterPanel(filterPanel, datafield); }, },
                  { text: 'Process Name', datafield: 'ProcessName', width: "20%", filtertype: "custom", createfilterpanel: function (datafield, filterPanel) { buildFilterPanel(filterPanel, datafield); }, },
                  { text: 'Conn Type', datafield: 'ConnTypeName', width: "20%", filtertype: 'checkedlist' },
                  { text: 'Primary', datafield: 'Primary', width: "20%", filtertype: "custom", createfilterpanel: function (datafield, filterPanel) { buildFilterPanel(filterPanel, datafield); }, },
                  { text: 'Secondary', datafield: 'Secondary', width: "20%",  filtertype: "custom", createfilterpanel: function (datafield, filterPanel) { buildFilterPanel(filterPanel, datafield); }, },
                ]
            });
            ...
            $("#jqxgrid").on('rowselect', function (event) {
                if (event.args.rowindex >= 0) {
                    var selectedRowData = $('#jqxgrid').jqxGrid('getrowdata', event.args.rowindex);
                    selectedRowId = selectedRowData["uid"];
                    selectedRowRef = selectedRowData["DSName"] + " (Process: " + selectedRowData["ProcessName"] + ")";
                }
            });
...
function EditRecord(Id) {
    if (Id == undefined) {
        ...
        $.ajax({
            type: "GET",
            url: "/DataSource/Edit?DataSourceId=" + Id,
            cache: false,
            success: function (result) {
                if (result.blnStatus != 'undefined' && result.blnStatus == false) {
                    alertDismissable(result.strType, result.strMessage);
...
function deleteTag(Id) {
    var activityName = "Delete Existing DataSource";
    var activityDetail = "DataSource: " + selectedRowRef;
    ...
    if (Id == undefined) {
        ...
        $('#deleteModal').modal('show');
        $('#myConfirmDelete').html("<p>Are you sure to Delete Datasource  : " + selectedRowRef + "?</p>");
        $("#confirmDelete").unbind('click');
        $("#confirmDelete").click(function () {
            $(".modal-backdrop").slideUp('slow');
            $.ajax({
                ...
                success: function (result) {
                    if (result.blnStatus != 'undefined' && result.blnStatus == false) {
                        ...
                        ShowPasswordModalForReqTrn(activityName + " : Requires Transaction Password", function () { deleteTagDo(Id); }, activityName, activityDetail);
                    }
                }
...
function deleteTagDo(Id) {

            $.ajax({
                type: "GET",
                url: "/DataSource/Delete?DataSourceId=" + Id,
                cache: false,
                success: function (result) {
                    alertDismissable(result.strType, result.strMessage);
