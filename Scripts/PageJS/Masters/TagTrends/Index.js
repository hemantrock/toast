...
var selectedRowId,TrendName, selectedRowRef;

$(document).ready(function () {

    $.ajax({
        type: "GET",
        url: "/TagTrends/IndexData",
        cache: false,
        beforeSend: function () { },
        complete: function () { },
        success: function (result) {
            ...
            var source =
            {
                ...
                datafields: [
                    ...
                    { name: 'TrendName', type: 'string' },
                    { name: 'ScanTime', type: 'number' },
                    { name: 'DurationName', type: 'string' },
            ...
            $("#jqxgrid").jqxGrid(
            {
                ...
                rendertoolbar: function (statusbar) {
                    ...
                    var tableHeading = $("<div style='float: left; margin-left: 5px;' class='heading-lbl'><span style='margin-left: 4px; position: relative; top: -3px;'>Tag Trends Master</span></div>");
                    ...
                    editButton.click(function (event) {
                        EditRecord(selectedRowId, trendName);
                    });

                    // delete Row.
                    deleteButton.click(function (event) {
                        deleteTagTrend(selectedRowId,trendName);
                    });
                ...
                columns: [
                  {
                      text: 'Tag Name', datafield: 'TagName', width: "20%",
                      filtertype: "custom",
                      createfilterpanel: function (datafield, filterPanel) {
                          buildFilterPanel(filterPanel, datafield);
                      },
                  },
                  { text: 'Tag Desc', datafield: 'TagDescription', width: "35%" },
                  { text: 'Trend Name', datafield: 'TrendName', width: "20%" },
                  { text: 'Scan Time', datafield: 'ScanTime', width: "10%", cellsalign: 'center', filtertype: 'checkedlist' },
                  { text: 'Duration', datafield: 'DurationName', width: "15%", cellsalign: 'center', filtertype: 'checkedlist' },
                  //{ text: 'Color', datafield: 'Color', width: "10%", cellsalign: 'center', filtertype: 'checkedlist' },
                ]
            });
            ...
            $("#jqxgrid").on('rowdoubleclick', function (event) {
                if (event.args.rowindex >= 0) {
                    ...
                    TrendName = selectedRowData["TrendName"];
                    EditRecord(selectedRowId, TrendName);
                }
            });
            ...
            $("#jqxgrid").on('rowselect', function (event) {
                if (event.args.rowindex >= 0) {
                    ...
                    selectedRowId = selectedRowData["uid"];
                    TrendName = selectedRowData["TrendName"];
                    selectedRowRef = selectedRowData["TagName"] + " (" + selectedRowData["TrendName"] + ")";
                }
            });
        },
...
function BindSpanDelete() {
    $('.spanDelete').on("click", function (e) {
        ...
        var trendName = $(this).attr('data-trendname');
        $("#confirmDelete").unbind('click');
        $("#confirmDelete").click(function () {
            $(".modal-backdrop").slideUp('slow');
            deleteTagTrend(Id, trendname);
            $('#deleteModal').modal('hide');
        });
    });
}

function OpenAddTagTrend()
{
    $.ajax({
        type: "GET",
        url: "/TagTrends/Add",
...
function EditRecord(Id, trendName)
{
    $.ajax({
        type: "GET",
        url: "/TagTrends/Edit?TagName=" + Id + "&TrendName="+trendName,
        cache: false,
        success: function (result) {
            if (result.blnStatus != 'undefined' && result.blnStatus == false) {
                alertDismissable(result.strType, result.strMessage);
...
function deleteTagTrend(Id, trendName) {
    var activityName = "Delete Existing TagTrend";
    ...
    if (Id == undefined) {
        ...
        $('#deleteModal').modal('show');
        $('#myConfirmDelete').html("<p>Are you sure to Delete Tag Trend : " + selectedRowRef + "?</p>");
        $("#confirmDelete").unbind('click');
        $("#confirmDelete").click(function () {
            $(".modal-backdrop").slideUp('slow');
            $.ajax({
                ...
                success: function (result) {
                    if (result.blnStatus != 'undefined' && result.blnStatus == false) {
                        deleteTagTrendDo(Id, trendName);
                    }
                    else {
                        ShowPasswordModalForReqTrn(activityName + " : Requires Transaction Password", function () { deleteTagTrendDo(Id, trendName); }, activityName, activityDetail);
                    }
                }
...
function deleteTagTrendDo(Id, trendName) {
            $.ajax({
                type: "GET",
                url: "/TagTrends/Delete?TagName=" + Id + "&TrendName=" + trendName,
                cache: false,
                success: function (result) {
                    alertDismissable(result.strType, result.strMessage);
