var selectedRowId, selectedRowRef, bayId, BCNo, GridWidth="1436px", HideCleanStart=true;
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            var cellsrendererNonCommMode = function (row, columnfield, value, defaulthtml, columnproperties) {
                var selectedRowData = $('#jqxgrid').jqxGrid('getrowdata', row);
                LoadingPointNo = selectedRowData["LoadingPointNo"];
                return '<div onclick="SetNonCommMode(' + LoadingPointNo + ')" style="text-align: center; display: inline-block; cursor: pointer; width: 100%; height: 100%; margin: 0 auto; position: relative;"><span style="width: 16px; height: 15px; margin: auto; position: absolute; top: 0; bottom: 0; left: 0; right: 0;" class="glyphicon glyphicon-check clickable spanAck"></span></div>';
            }
            ...
            $("#jqxgrid").jqxGrid(
            {
                ...
                columns: [
                  { text: 'BayID', datafield: 'BayID', width: "75px", filtertype: 'checkedlist', cellsalign: 'center' },
                  { text: 'ArmNo', datafield: 'ArmNo', filtertype: 'checkedlist', width: "75px", cellsalign: 'center' },
                  { text: 'LP No', datafield: 'LoadingPointNo', width: "75px", cellsalign: 'center' },
                  { text: 'BC No', datafield: 'BCNo', width: "75px", cellsalign: 'center' },
                  { text: 'Products', datafield: 'ProductNames', width: "537px" },
                  { text: 'LP Enabled', datafield: 'LPStatus', threestatecheckbox: true, columntype: 'checkbox', filtertype: 'checkedlist', width: "87px" },
                  { text: 'Maint', width: "75px", cellsalign: 'center', cellsrenderer: cellsrendererNonCommMode },
                  { text: 'AcuID', datafield: 'AcuID', width: "87px", cellsalign: 'center' },
                  { text: 'Loading Type', datafield: 'LoadingType', filtertype: 'checkedlist', width: "125px", cellsalign: 'center' },
                  { text: 'Purpose', datafield: 'LoadPurposeDesc', filtertype: 'checkedlist', width: "125px", cellsalign: 'center' },
                  { text: 'OvspBypass', datafield: 'IsOverspillBypassed', threestatecheckbox: true, columntype: 'checkbox', filtertype: 'checkedlist', width: "100px" },
                  { text: 'Clean Start', width: "75px", cellsalign: 'center', cellsrenderer: cellsrendererClearStart, hidden: HideCleanStart },
                ]
            });
...
function SetNonCommMode(LoadingPointNo) {
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.blnStatus != 'undefined' && result.blnStatus == false) {
                SetMaintModeDo(LoadingPointNo);
            }
            else {
                ShowPasswordModalForReqTrn(activityName + " : Requires Transaction Password", function () { SetNonCommModeDo(LoadingPointNo); }, activityName, activityDetail);
            }
        }
    });
}

function SetNonCommModeDo(LoadingPointNo) {
    $.ajax({
        type: "GET",
        url: "/BatchController/SetBCUArmNonCommMode?LoadingPointNo=" + LoadingPointNo,
        cache: false,
        success: function (result) {
            alertDismissable(result.strType, result.strMessage);
        }
